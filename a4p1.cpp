...
void execute1a4(ofstream &out)
{
    while (pc <= maxInstructions)
    {
        ...
        { //add
            ...
            if (cannotUseRegistersG.find(currInst.cirs[0]) == cannotUseRegistersG.end() 
                && cannotUseRegistersG.find(currInst.cirs[1]) == cannotUseRegistersG.end()
                && cannotUseRegistersG.find(currInst.cirs[2]) == cannotUseRegistersG.end()
                && cannotChangeRegistersG.find(currInst.cirs[0]) == cannotUseRegistersG.end()) 
            {
                add(currInst.cirs);
                ...
                numOfInst[0]++;
                 ...
                 if (cannotUseRegistersG.find(currInst.cirs[0]) != cannotUseRegistersG.end()) {
                    int insAdd = BlockRegToInstAdd[currInst.cirs[0]];
                    if (iset[insAdd].opID == 7)//lw 
                    {
                        lw ()
                    }
                    else if (iset[insAdd].opID == 8) {

                    }
                 }
                 if (cannotUseRegistersG.find(currInst.cirs[1]) != cannotUseRegistersG.end()) {
                    int insAdd = BlockRegToInstAdd[currInst.cirs[0]];
                    if (iset[insAdd].opID == 7)//lw 
                    {
                ...
                if (cannotUseRegistersG.find(currInst.cirs[2]) != cannotUseRegistersG.end()) {
                    int insAdd = BlockRegToInstAdd[currInst.cirs[0]];
                    if (iset[insAdd].opID == 7)//lw 
                    {
                ...
                else
                {
                    ...
                    // assume that inside bracket always register. 
                        ...
                        if (cannotChangeRegistersG.find (currInst.cirs[0]) == cannotChangeRegistersG.end() && ) {
                            ...
                            BlockRegToInstAdd[currInst.cirs[0]] = pc;
                            BlockRegToInstAdd[currInst.cirs[2]] = pc;
                            BlockMemoryToInstAdd[address] = pc;
                        }
